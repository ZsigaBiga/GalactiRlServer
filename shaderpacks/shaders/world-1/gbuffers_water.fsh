...
vec3 GetWaterParallaxCoord(in vec3 position, in vec3 viewVector)
{
	...
	vec3 stepSize = vec3(0.2f * 0.0f, 0.2f * 0.0f, 0.2f);
...
vec3 GetWavesNormal(vec3 position, in float scale, in mat3 tbnMatrix) {
	...
	vec3 wavesNormal;
		 ...
		 wavesNormal.r *= 30.0f * 0.0f / sampleDistance;
		 wavesNormal.g *= 30.0f * 0.0f / sampleDistance;
...
void main() {
	...
	if (iswater > 0.5f && !backfacing) {
		...
		tex = vec4(0.0f, 0.01f, 0.3f, 200.0f/255.0f);
