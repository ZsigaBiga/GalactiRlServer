...
#define NORMAL_MAP_MAX_ANGLE 1.3f   		//The higher the value, the more extreme per-pixel normal mapping (bump mapping) will be.
...
void main() {
	...
	else
	{
		...
		wet = clamp(wet * 1.5f - 0.2f, 0.0f, 1.0f);
		 spec.g += max(0.0f, clamp((wet * 0.1f + 0.2f), 0.0f, 1.0f) - (1.0f - w) * 1.0f);
