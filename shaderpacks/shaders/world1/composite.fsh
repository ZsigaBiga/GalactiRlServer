...
const float 	ambientOcclusionLevel 	= 0.85f;
...
void 	AddSkyGradient(inout SurfaceStruct surface) {
	...
	surface.sky.albedo *= mix(1.0f, 0.5f, timeNoon);
...
void main() {
	...
	surface.sky.tintColor   = vec3(31.0f, 0.0f, 57.0f);
...
//s	AddSunglow(surface);
	...
	#ifdef NightEye
		// nighteye effect on outdoor lighting and sky
	...
	vec3 finalComposite = final.sunlight 			* 0.55f 	* sunlightMult				//Add direct sunlight
						...
						+ final.nolight 			* 0.025f			//Add base ambient light
	...
	//Apply sky to final composite
		 surface.sky.albedo *= 0.5f;
		 surface.sky.albedo = surface.sky.albedo * surface.sky.tintColor;
		 //CloudPlane(surface);
		 //DoNightEye(surface.sky.albedo);
...
//	CalculateAtmosphericScattering(finalComposite.rgb, surface);
	...
	vec3 fogColor = vec3(0.104, 0.05, 0.104);

//finalComposite = mix(finalComposite, fogColor, min(GetDepthLinear(texcoord.st) / far * density , 1.0));
finalComposite = mix(finalComposite, fogColor, min(GetDepthLinear(texcoord.st) / far * 0.045 , 1.0));
